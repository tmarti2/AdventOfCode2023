...
let filter_cell_map ~f a p = filter_aux ~get:get_map ~adj:neightbors_cells_map ~f a p
...
(* let find_aux ~f ~adj ~get a p =
  let f = Option.map ~f in
  List.find_map (adj p) ~f:(f << get a)

let find_neightbor a p = find_aux ~adj ~get a p
let find_neightbor_map a p = find_aux ~adj:adj_map ~get:get_map a p *)